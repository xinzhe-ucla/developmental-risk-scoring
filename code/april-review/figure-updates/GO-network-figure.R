...
for (disease in c('SCZ', 'ADHD')){
    for (combo in combos){
        ...
        cor_collection[[name]] = readRDS(paste0('/u/home/l/lixinzhe/project-geschwind/port/scratch/revision/GO/age/2026-06-12_', name, '_score_mcg_correlation.rds'))
        go_collection[[name]] = readRDS(paste0('/u/home/l/lixinzhe/project-geschwind/port/scratch/revision/GO/age/2026-06-12_', name, '_readable_go_results.rds'))
    }
}
...
for (combo in names(cor_collection)){
    ...
    plot.name <- paste0(network.path, Sys.Date(), '-', combo, '-gene-ontology-enrichment-network-legend.pdf')
    pdf(
        file = plot.name,
        width = 10,
        height = 10
    ...
    plot.name <- paste0(network.path, Sys.Date(), '-', combo, '-gene-ontology-enrichment-network.pdf')
